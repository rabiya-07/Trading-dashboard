...
@st.cache_data
def load_data(trades_source, sentiment_source):
    trades = pd.read_csv(trades_source)
    sentiment = pd.read_csv(sentiment_source)
...
uploaded_sentiment = st.sidebar.file_uploader("Upload sentiment.csv", type="csv")
trades_url = _get_secret("TRADES_CSV_URL", "CSV_URL")
sentiment_url = _get_secret("SENTIMENT_CSV_URL")
...
if uploaded_sentiment is not None:
    sentiment_source = uploaded_sentiment
elif sentiment_url:
    sentiment_source = sentiment_url
elif SENTIMENT_FILE.exists():
    sentiment_source = SENTIMENT_FILE
else:
    st.info(
        "Upload sentiment.csv in the sidebar, add a SENTIMENT_CSV_URL secret in "
        "Streamlit Cloud, or keep sentiment.csv in GitHub with the app."
    )
    st.stop()

if st.sidebar.button("Refresh uploaded/remote CSV data"):
    load_data.clear()

merged = load_data(trades_source, sentiment_source)
